...
public class SensorControler implements IActivityLifiCycle, SensorEventListener {
  ...
  public static final int DELEY_DURATION_PLUS = 1500;
  ...
  private SensorControler() {
    mSensorManager =
        (SensorManager) BaseApplication.getContext().getSystemService(Activity.SENSOR_SERVICE);
  ...
  @Override public void onSensorChanged(SensorEvent event) {
    ...
    if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
      ...
      if (STATUE != STATUS_NONE) {
        ...
        Log.d(TAG,
            "pX:" + px + "  pY:" + py + "  pZ:" + pz + "    stamp:" + stamp + "  second:" + second);
        ...
        if (value > 1.4) {
          ...
          if (STATUE == STATUS_MOVE) {
            lastStaticStamp = stamp;
            Log.i(TAG, "mobile static 1");
          } else if (STATUE == STATUS_STATIC) {
            Log.i(TAG, "mobile static 2");
            if (stamp - lastStaticStamp > DELEY_DURATION) {
              canFocusIn = true;
              Log.i(TAG, "mobile static 3");
            }
          }

          if (canFocusIn) {
            if (stamp - lastStaticStamp > DELEY_DURATION_PLUS) {
              //移动后静止一段时间，可以发生对焦行为
              canFocus = false;
              Log.i(TAG, "mobile static 4");
              if (!isFocusing) {
                canFocusIn = false;
                // onCameraFocus();
                if (mCameraFocusListener != null) {
                  mCameraFocusListener.onFocus();
                  Log.i(TAG, "mobile static 5");
                }
  ...
  public boolean isFocusLocked() {
    if (canFocus) {
      System.out.println("foucsing: "+foucsing);
      return foucsing <= 0;
    }
    return false;
  }
