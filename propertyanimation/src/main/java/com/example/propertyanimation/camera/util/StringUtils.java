package com.example.propertyanimation.camera.util;
...
//    }
    ...
    public static Bitmap stringToBitmap(String text, int size, int color, Typeface font, int bgColor) {
        ...
        float space =  BaseApplication.getContext().getResources().getDimensionPixelSize(R.dimen.x2);  //背景padding

        int textHeight = (int) getTextHeight(p);    //字的高度
        int pixLen = (int) (getFontHeight(p) + 2* space);//正方形边长

        int offsetY =   BaseApplication.getContext().getResources().getDimensionPixelSize(R.dimen.x2);    //y方向上的偏移  手动调整
    ...
    public static float getLineSpaceHeight(){
        return   BaseApplication.getContext().getResources().getDimensionPixelSize(R.dimen.x4);
    }
    ...
    public static float getCenterY(Paint paint){
        int offset =  BaseApplication.getContext().getResources().getDimensionPixelSize(R.dimen.x4);
