...
public class CameraManager implements ICameraHelper {
    ...
    private ImageView m_tvFlashLight, m_tvCameraDireation;  //绑定的闪光灯和前后置镜头切换控件
    ...
    public static final int[] RES_DRAWABLE_FLASHLIGHT = { R.mipmap.smart_camera_flash, R.mipmap.smart_camera_flash_auto, R.mipmap.smart_camera_flash_close};
    public static final int[] RES_DRAWABLE_CAMERA_DIRECTION = {R.mipmap.smart_camera_lenses, R.mipmap.smart_camera_lenses};
    ...
    private CameraManager(Context context) {
        ...
        mLightStatus = FlashLigthStatus.valueOf(
            SPConfigUtil.loadInt(SP_LIGHT_STATUE, FlashLigthStatus.LIGHT_AUTO.ordinal())); //默认 自动
     ...
     */
    public void bindOptionMenuView(ImageView tvFlashLight, ImageView tvCameraDireation) {
    ...
    private Camera.Size findFitPicResolution(Camera camera, float bl) throws Exception {
        ...
        //Camera.Size picSize = getOptimalPreviewSize(sizes, MyApplication.getWindowWidth(),MyApplication.getWindowHeight());
    ...
    private Camera.Size findFitPreResolution(Camera camera) throws Exception {
        ...
        //Camera.Size showSize = getOptimalPreviewSize(ShowSizes,MyApplication.getWindowWidth(),MyApplication.getWindowHeight());
        //cameraParameters.setPreviewSize(showSize.width, showSize.height);
        //System.out.println(showSize.width+" : "+showSize.height);
        //return showSize;

    }

    private Camera.Size getOptimalFuckSize(List<Camera.Size> sizes, int w,
        int h) {
        double targetRatio=(double)w*1.0 / h;
        ...
        for (int i = 0; i < sizes.size(); i++) {
            ...
            if (size.height == 1080 && size.width == 1920){
                return size;
            }else if (size.height == w && size.width == h){
                return size;
            }else {
                double xx = (double)size.height*1.0/size.width;
                if (Math.abs(xx - targetRatio)<asdf){
                    asdf = Math.abs(xx - targetRatio);
                    optimalSize = size;
                }
            }
        }
     ...
     */
    public Camera.Size findBestResolution(Camera cameraInst, double maxDistortion, int type)  throws
        Exception {
        Camera.Parameters cameraParameters = cameraInst.getParameters();
        List<Camera.Size> supportedPicResolutions = type == TYPE_PREVIEW ? cameraParameters.getSupportedPreviewSizes() : cameraParameters.getSupportedPictureSizes(); // 至少会返回一个值
    ...
    public void openCameraActivity(Context context) {
        Intent intent = new Intent(context, SmartCameraActivity.class);
    ...
    public void setFuckFitPreAndPicSize(Camera mCamera) {
        System.out.println(BaseApplication.getWindowWidth()+"  "+BaseApplication.getWindowHeight());
        Camera.Parameters mParams = mCamera.getParameters();
        List<Camera.Size> sizes = mParams.getSupportedPictureSizes();
        Camera.Size picSize = getOptimalFuckSize(sizes, BaseApplication.getWindowWidth(),BaseApplication.getWindowHeight());
        mParams.setPictureSize(picSize.width, picSize.height);
        List<Camera.Size> ShowSizes = mParams.getSupportedPreviewSizes();
        Camera.Size showSize = getOptimalFuckSize(ShowSizes,BaseApplication.getWindowWidth(),BaseApplication.getWindowHeight());
        mParams.setPreviewSize(showSize.width, showSize.height);
