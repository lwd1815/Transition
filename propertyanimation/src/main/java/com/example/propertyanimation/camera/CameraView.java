...
public class CameraView extends SurfaceView
    implements SurfaceHolder.Callback, ICameraOperation, IActivityLifiCycle {
  ...
  private void initCamera() {
    ...
    //
    //Log.i(TAG,
    //    "adpterSize Preview-->width:" + adapterSize.width + "  height:" + adapterSize.height);
    //
    //adapterSize = mCamera.getParameters().getPictureSize();
    //Log.i(TAG,
    //    "adpterSize Picture-->width:" + adapterSize.width + "  height:" + adapterSize.height);

    //调整控件的布局  防止预览被拉伸
    //adjustView(adapterSize);

    mCameraManager.setFuckFitPreAndPicSize(mCamera);
    Camera.Size adapterSize = mCamera.getParameters().getPreviewSize();
    Log.i(TAG,
        "adpterSize Preview-->width:" + adapterSize.width + "  height:" + adapterSize.height);
    adapterSize = mCamera.getParameters().getPictureSize();
  ...
  @Override public void switchFlashMode(int i) {
    switch (i) {
      case 0:
  ...
  @Override public boolean takePicture() {
    Log.e("take picture :",System.currentTimeMillis()+"");
    try {
  ...
  protected boolean focus(Camera.AutoFocusCallback callback) {
    try {
  ...
  public void setOneShotPreview(Camera.PreviewCallback previewCallback) {
